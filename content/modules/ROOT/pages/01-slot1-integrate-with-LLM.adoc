...
To interact with the Mistral model running on OpenShift AI, we’ll include a specific dependency in the pom.xml file located in `triage-application`. Our model, deployed as vLLMfootnote:[Deploying an LLM using Openshift AI[01-slot1-install-openshift-ai.adoc]], exposes an OpenAI API. Therefore, we are using the Quarkus OpenAI dependency to interact with it.
...
[NOTE]
====
This dependency is specific to OpenAI and models deployed with vLLM. Quarkus can interact with many other LLMs.
====
...
[source, xml]
----
$ export MISTRAL_AI_BASE_URL={llm-base-url}
$ ./mvnw quarkus:dev
----
...
quarkus.langchain4j.openai.base-url={llm-base-url}
quarkus.langchain4j.openai.api-key=<key>
quarkus.langchain4j.openai.chat-model.model-name={model-name}
----
...
One of the great benefits of OpenShift AI is that it allows you to deploy multiple models. If you are using vLLM, all you need to do is switch the link to another model.

If you are using a composite Caikit and TGIS, you should use the Quarkus OpenShift AI extension.
